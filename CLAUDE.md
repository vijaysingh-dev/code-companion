...
- **Sessions**: conversation history is server-side (not client-supplied). `ChatRequest` = `session_id` + `message` (+ optional `context`/`provider`/`model`/`effort`). `POST /api/chat` loads the thread, streams, and persists the user + assistant turns; history is compacted server-side (a char budget for now). ChatSession CRUD lives at `/api/sessions` (all authenticated, scoped to the caller); `PATCH /api/sessions/{id}` renames. `SessionService` owns persistence + domain-message conversion. Auto-title: on the first turn of an untitled session the `mini_model` summarizes the message into a title, streamed as a `TITLE` SSE event (`app/services/chat.py::_generate_title`, prompt `app/llm/prompts/title.json`).
- **Turn-1 flow (target, NOT built — tool loop is a prereq)**: the intended shape once the agent tool loop lands is a **mini-first pre-pass**. On the first message the mini tier runs first with a dynamically-augmented system prompt (title generation + "gather context: ask the user a clarifying question or call read/search tools until context is complete"). Only once context is sufficient does the **main** tier get invoked to create/edit files. Today only the title half exists and it runs *after* the main turn; reorder to before + add the context-gathering loop when Phase 2 (agent loop) arrives.
