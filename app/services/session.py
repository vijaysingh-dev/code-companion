...
class SessionService:
    ...
    async def create(self, user_id: str, provider: str, model: str, title: str | None = None) -> Session:
        row = Session(user_id=user_id, provider=provider, model=model, title=title)
    ...
    async def get(self, user_id: str, session_id: str) -> Session:
        row = await self._session.get(Session, session_id)
        if row is None or row.user_id != user_id:
            raise ResourceNotFoundError("Session", details={"session_id": session_id})
        return row

    async def list_all(self, user_id: str) -> list[Session]:
